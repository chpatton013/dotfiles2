...
Process the contents of `.agents/workspace/INBOX.md` and incorporate them into
`.agents/workspace/TODO.md`. This is a triage-only command: move + id + clear.
Do NOT start doing the tasks themselves unless separately instructed.

If `.agents/workspace/` doesn't exist yet, or this project already tracks its
backlog somewhere else (e.g. `docs/followups.md`), stop and check the
`organizing-workspace` skill before proceeding — it defines the convention this
command depends on and covers first-time setup and how to recognize an
existing project-specific convention that should be used instead.
